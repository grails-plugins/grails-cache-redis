...
target(createCacheRedisTestApps: 'Creates test apps for functional tests') {
...
private void init(String name, config) {
	...
	pluginZip = new File(basedir, "grails-cache-redis-${pluginVersion}.zip")
	if (!pluginZip.exists()) {
...
setDefaultTarget 'createCacheRedisTestApps'
